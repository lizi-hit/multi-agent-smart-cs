...
class SupervisorNode:
    ...
    async def route_decision(self, state: AgentState) -> AgentState:
        """基于intent_router结果做路由，必要时回退到Supervisor LLM判定"""
        ...
        # 若intent_router结果异常，回退到Supervisor自主判定，保证可用性
...
def _build_fallback_result(node_name: str) -> str:
    if node_name == "knowledge_rag":
        return "当前知识检索服务繁忙，已记录您的问题并转人工跟进。"
    if node_name == "ticket_handler":
        return "工单服务暂时不可用，已为您生成转人工请求，请稍后重试。"
    if node_name == "compliance_check":
        return "合规审查超时，系统已转人工审核以保障安全。"
        ...
        for attempt in range(max_retries + 1):
            try:
                ...
                if attempt >= max_retries:
                    fallback = _build_fallback_result(node_name)
                    sub_results = dict(state.get("sub_results", {}))

                    if node_name == "compliance_check":
                        # 合规节点失败时采用保守策略：判定不通过并转人工
                        sub_results["compliance"] = {
                            "passed": False,
                            "risk_level": "high",
                            "violations": [f"compliance_timeout: {str(last_error)}"],
                        }
                        return {
                            **state,
                    ...
                    sub_results[node_name] = fallback
                    return {
                        **state,
                        "sub_results": sub_results,
                    }
...
def create_supervisor_graph(
    ...
    graph.add_node("intent_router", with_timeout_retry("intent_router", intent_router.process))
    graph.add_node("supervisor_route", with_timeout_retry("supervisor_route", supervisor.route_decision))
    graph.add_node("knowledge_rag", with_timeout_retry("knowledge_rag", knowledge_agent.process))
    graph.add_node("ticket_handler", with_timeout_retry("ticket_handler", ticket_agent.process))
    graph.add_node("compliance_check", with_timeout_retry("compliance_check", compliance_agent.process))
